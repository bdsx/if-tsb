...
export class ConcurrencyQueue {
    ...
    private readonly _next:(lastTask:TaskObject|null)=>(Promise<void>|void) = (lastTask:TaskObject|null)=>{
        if (lastTask !== null) {
        ...
        return task.task().then(()=>this._next(task), err=>this.error(err));
    };
    ...
    run(name:string, task:()=>Promise<void>):Promise<void> {
        ...
        if (this.allTasks.size >= this.concurrency) {
            if (this.reserved.length > (this.concurrency>>1)) {
                if (ConcurrencyQueue.verbose) console.log(`${this.name} - Drain`);
